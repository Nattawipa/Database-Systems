...
BEGIN
    ...
    SELECT * FROM Customer WHERE CustomerNum = @custNum;
END
GO
...
BEGIN
    ...
    FROM Customer WHERE CustomerNum = @custNum;
END
GO
...
AS  
    ...
    IF EXISTS (SELECT * FROM Customer WHERE CustomerNum = @custNum)  
        BEGIN  
            SET  @resultValue = -5  
        END  
    ELSE  
        BEGIN  
            INSERT INTO Customer (CustomerNum, CustomerName)  
            VALUES (@custNum, @custName)
            SET @resultValue = 0  
        END  
...
CREATE PROCEDURE sp_insert_order  
    @amount			INT,         -- amount of parts
    @part			NVARCHAR(10),  -- part number
	@customerNum	INT     -- customer number
AS  
BEGIN  

	DECLARE @quotedPrice MONEY
    IF EXISTS (SELECT Part.Price FROM Part)  
        BEGIN  
            SET  @quotedPrice = @amount * (SELECT Part.Price FROM Part WHERE Part.PartNum = @part)
        END 

	DECLARE @nextOrderNum INT 
    IF EXISTS (SELECT OrderNum FROM Orders)  
        BEGIN
			SET  @nextOrderNum = (SELECT MAX(OrderNum) from Orders ) +1
        END  

	INSERT INTO Orders (OrderNum, OrderDate, CustomerNum) VALUES (@nextOrderNum, GETDATE() , @customerNum)

	INSERT INTO OrderLine (OrderNum, PartNum, NumOrdered, QuotedPrice) VALUES (@nextOrderNum, @part, @amount, @quotedPrice)
...
VALUES ('011','Sarah Mogan', 5000, 4);

-- Step 2: try to insert again with valid input
INSERT INTO person (pid, full_name, budget, proj_id) 
VALUES ('011','Sarah Mogan', 15000, 4);
...
SELECT * FROM person WHERE proj_id = 4
GO

SELECT * FROM project WHERE proj_id = 4
GO

SELECT SUM(budget) AS New_Total_Budget
FROM person WHERE proj_id = 4
GO
...
BEGIN
    ...
    SET @sumbudget = (SELECT SUM(budget) 
                      FROM [person] WHERE proj_id = @projID )
...
SELECT * FROM person WHERE pid = '001';     -- budget = 20000
SELECT * FROM person WHERE proj_id = 1;     -- 3 persons
SELECT * FROM project WHERE proj_id = 1;    -- total budget = 55000
...
-- Trigger is executed, and data are still consistent.
SELECT * FROM person WHERE proj_id = 1;     -- 3 persons
SELECT * FROM project WHERE proj_id = 1;    -- total budget = 45000
GO
...
BEGIN
	...
	SET @budget = (SELECT budget from INSERTED)
	SET @project_ID = (SELECT proj_id from INSERTED)
...
INSERT INTO person (pid, full_name, budget, proj_id)
VALUES ('012','Harry Potter', 15000, 5);
...
BEGIN
	...
	SET @logId = (SELECT LogID from LogFileAudit ) 
	IF @logId IS NULL
		SET @logId = 0
	ELSE
		SET @logId = (SELECT MAX(LogID) from LogFileAudit ) + 1

	INSERT INTO LogFileAudit VALUES (@logId,'INSERT','person')
	
END  

-- test 
INSERT INTO person VALUES ('089','Nattawipa Saetae', 270743.27, 7, GETDATE())
...
BEGIN
	...
	INSERT INTO LogFileAudit VALUES (@logId,'DELETE','project')
